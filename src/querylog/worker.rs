...
pub async fn run_qps_sampler(
    ...
    use super::ring::{StatsSample, STATS_SAMPLE_SECS};
    ...
    let mut prev_queries = counters.queries_total();
    ...
    // The QPS ring keeps per-second detail, but the stats ring stores one
    // sample per `STATS_SAMPLE_SECS` seconds — accumulate the per-second deltas
    // here and push a folded sample when the interval elapses.
    let mut stats_acc = StatsSample::default();
    let mut ticks_this_sample = 0usize;
    ...
    loop {
        tokio::select! {
            _ = ticker.tick() => {
                ...
                let cur_queries  = counters.queries_total();
                ...
                // Fold this second's deltas into the current stats sample; push
                // it to the ring once STATS_SAMPLE_SECS seconds have elapsed.
                stats_acc.queries += snap.queries;
                stats_acc.cache_hits += snap.cache_hits;
                stats_acc.upstream_ok += snap.upstream_ok;
                stats_acc.upstream_err += snap.upstream_err;
                stats_acc.filtered += snap.filtered;
                ticks_this_sample += 1;
                if ticks_this_sample >= STATS_SAMPLE_SECS {
                    stats_acc.unix_secs = now_secs;
                    stats_ring.push(stats_acc);
                    stats_acc = StatsSample::default();
                    ticks_this_sample = 0;
                }
...
/// Await the next event from any shard, then opportunistically drain more (up to
/// `batch_size`) from every shard without awaiting. `start` rotates which shard
/// is polled first so no shard is starved. Returns `None` only when every shard
/// is closed and empty (all senders dropped).
async fn recv_batch(
    rxs: &mut [mpsc::Receiver<QueryLogEvent>],
    start: &mut usize,
    batch_size: usize,
) -> Option<Vec<QueryLogEvent>> {
    let n = rxs.len();
    let first = poll_fn(|cx| {
        ...
        for k in 0..n {
            ...
            match rxs[idx].poll_recv(cx) {
                ...
                Poll::Ready(None) => {}           // this shard is closed
                Poll::Pending => any_open = true, // still has live senders
            }
        }
        if any_open {
            Poll::Pending
    ...
    batch.push(first);
    drain_available(rxs, start, batch_size, &mut batch);
    Some(batch)
}

/// Non-awaiting drain of whatever is currently buffered across all shards,
/// appending to `batch` up to `batch_size`.
fn drain_available(
    rxs: &mut [mpsc::Receiver<QueryLogEvent>],
    start: &mut usize,
    batch_size: usize,
    batch: &mut Vec<QueryLogEvent>,
) {
    let n = rxs.len();
...
pub async fn run(
    mut rxs: Vec<mpsc::Receiver<QueryLogEvent>>,
    ring: Arc<EventRing>,
    counters: Arc<QueryLogCounters>,
    ...
    // Rotates which shard is polled first each wakeup so no shard is starved.
    let mut start_shard = 0usize;

    loop {
        tokio::select! {
            batch = recv_batch(&mut rxs, &mut start_shard, batch_size) => {
                match batch {
                    Some(batch) => {
                        process_batch(batch, &ring, &counters, &file_cfg, &mut file_state).await;
                    }
                    // Every shard closed (all senders dropped): nothing more can arrive.
                    None => {
            ...
            changed = shutdown.changed() => {
                if changed.is_err() || *shutdown.borrow() {
                    // Drain all remaining events across every shard before returning.
                    loop {
                        let mut batch = Vec::with_capacity(batch_size);
                        drain_available(&mut rxs, &mut start_shard, batch_size, &mut batch);
                        if batch.is_empty() {
                            break;
                        }
                        process_batch(batch, &ring, &counters, &file_cfg, &mut file_state).await;
...
async fn process_batch(
    ...
    // Wrap each event in Arc, then push the whole batch to the ring under a
    // single lock acquisition (see `EventRing::push_batch`).
    let arc_batch: Vec<Arc<QueryLogEvent>> = batch.into_iter().map(Arc::new).collect();
    if ring.enabled() {
...
impl MsgpackFileState {
    ...
    async fn append_batch(
        ...
        if self.bytes_written >= cfg.max_mb.saturating_mul(1_048_576) {
...
async fn prune_old_files(cfg: &QueryLogFileConfig) -> anyhow::Result<()> {
    ...
    if let Some(days) = cfg.retention_days {
        ...
        let cutoff = now_micros.saturating_sub((days as u64).saturating_mul(86_400_000_000));
...
fn read_legacy_history_page(path: &Path, query: &HistoryQuery) -> anyhow::Result<HistoryPage> {
    ...
    let max_scan: u64 = if query.qname.as_deref().is_some_and(|q| !q.is_empty())
        ...
        || query.upstream.as_deref().is_some_and(|s| !s.is_empty())
        || query.client.as_deref().is_some_and(|s| !s.is_empty())
        || query.qtype.is_some()
...
fn event_matches(event: &DecodedEvent, query: &HistoryQuery) -> bool {
    query
        ...
        && query.upstream.as_deref().is_none_or(|u| {
            u.is_empty() || event.upstream.as_deref().is_some_and(|eu| eu.contains(u))
        })
        && query
            .client
            .as_deref()
            .is_none_or(|c| c.is_empty() || event.client.contains(c))
        && query.qtype.is_none_or(|t| event.qtype == t)
}
