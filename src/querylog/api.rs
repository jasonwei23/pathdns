    ...
    let rules: Vec<_> = hot
        ...
        .map(|g| {
            let tags: Vec<_> = g
                ...
                .chain(g.ruleset_exclude.iter().map(|t| {
                    serde_json::json!({
                        "tag": t,
                        "include": false,
                        "count": tag_counts.get(t.as_str()).copied().unwrap_or(0),
            ...
            let filters: Vec<_> = g
                ...
                .map(|f| {
                    let action = match f.action {
                        crate::response_filter::FilterAction::Empty => "empty".to_string(),
                        crate::response_filter::FilterAction::Drop => "drop".to_string(),
                        crate::response_filter::FilterAction::Continue => "continue".to_string(),
                        crate::response_filter::FilterAction::Forward(idx) => format!(
                            "forward:{}",
                            hot.rules.get(idx).map(|r| r.name.as_str()).unwrap_or("?")
                        ),
                    };
