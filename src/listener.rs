...
pub async fn serve_udp(bind: SocketAddr, iface: Option<&str>, state: Arc<AppState>) -> Result<()> {
    let (buf_size, n, batch_size, diagnostics) = {
    ...
    for _ in 0..n {
        sockets.push(Arc::new(
            bind_udp_socket_reuse_port(bind, buf_size, iface, diagnostics)
...
async fn handle_tcp_conn(
    ...
    // Fast-path response arena (see `resolver::try_fast_path_into`) plus
    // reusable body/framing buffers. The arena chunk is deliberately smaller
    // than the UDP shards' — it is held per connection, and `tcp_max_connections`
    // of them can be alive at once.
    const TCP_ARENA_CHUNK: usize = 8 * 1024;
    let mut resp_buf = ResponseArena::new(TCP_ARENA_CHUNK);
...
fn bind_udp_socket_reuse_port(
    ...
    // Best-effort socket-level cmsgs delivered with every datagram, gated by
    // `runtime.udp-diagnostics` (each requested cmsg costs the kernel a
    // control-message build and userspace a parse per datagram):
    //  - SO_RXQ_OVFL (basic+): cumulative receive-buffer overflow drop count
    //  - SO_TIMESTAMPNS (full): kernel receive timestamp, for kernel→userspace latency
    let yes: libc::c_int = 1;
    if diagnostics >= UdpDiagnostics::Basic {
        let _ = sys::set_socket_i32(fd.as_raw_fd(), libc::SOL_SOCKET, libc::SO_RXQ_OVFL, yes);
    }
    if diagnostics >= UdpDiagnostics::Full {
        let _ = sys::set_socket_i32(fd.as_raw_fd(), libc::SOL_SOCKET, libc::SO_TIMESTAMPNS, yes);
    }
...
/// Bind a TCP listener with deterministic IPv6 behaviour (IPV6_V6ONLY set) and
/// `SO_REUSEPORT` set, so `serve_tcp`'s per-shard listeners (and the dashboard
/// HTTP API's single listener) can all bind the same address concurrently.
/// `iface` is applied via SO_BINDTODEVICE when set.
...
fn create_reuse_port_socket(
    ...
    if addr.is_ipv6() {
        sys::set_socket_i32(fd.as_raw_fd(), libc::IPPROTO_IPV6, libc::IPV6_V6ONLY, yes)
