...
impl NetlinkSocket {
    ...
    /// The timeout bounds the blocking `recv` used by set-membership tests and the
    /// background worker's ack drain so a missing reply can never hang a thread.
    ...
    pub(super) fn send_raw(&self, buf: &[u8]) -> Result<()> {
        let sent = sys::send(self.fd.as_raw_fd(), buf, 0)
