...
//!
//! `IpSetManager` exposes one operation: `add_rule_ips`, which enqueues IPs
//! for background addition to a named rule's ipset. `route.final`'s
//! primary/secondary decision is a separate, in-memory ipcidr-behavior
//! `route.ruleset` check — see `crate::iprange` and `RuleSetDb::matches_ip`.
//!
