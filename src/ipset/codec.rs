...
impl NlBuilder {
    ...
    fn nest(&mut self, kind: u16, f: impl FnOnce(&mut Self)) {
        ...
        f(self);
        let raw_len = self.buf.len() - start;
        // Callers are responsible for keeping batches small enough to fit (see
        // `MAX_IPS_PER_MESSAGE` in `ipset/client.rs`); this catches a regression
        // rather than silently truncating the length and corrupting the message.
        debug_assert!(
            raw_len <= u16::MAX as usize,
            "netlink nested attribute {raw_len} bytes exceeds u16::MAX — batch too large"
        );
