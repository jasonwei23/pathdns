...
fn cache_answer(
    ...
    let stripped_query: Option<Bytes> = variant
        ...
        .flatten();
    let cache_query: &[u8] = stripped_query
        .as_deref()
        .unwrap_or_else(|| ctx.packet.as_ref());
...
async fn resolve_none_rule_with_cidr(
    ...
    let hot = state.hot.load_full();
    let answer_ip_tags = match &hot.cfg.fallback.target {
        FallbackTarget::Dual { answer_ip_tags, .. } if !answer_ip_tags.is_empty() => {
            Some(answer_ip_tags.clone())
        }
        _ => None,
    };
