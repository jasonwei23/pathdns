...
mod collapse_tests {
    ...
    fn collapses_chain_to_single_record() {
        ...
        assert_eq!(answer_ips(&out, qend).as_slice(), [std::net::IpAddr::from([95, 100, 196, 60])]);
    }
    ...
    fn no_cname_is_left_untouched() {
        ...
        let mut p = Vec::new();
        p.extend_from_slice(&[0x12, 0x34, 0x81, 0x80, 0x00, 0x01, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00]);
        p.extend_from_slice(b"\x03www\x07example\x03com\x00");
        p.extend_from_slice(&[0x00, 0x01, 0x00, 0x01]);
        let qend = p.len();
        p.extend_from_slice(&[0xC0, 0x0C, 0x00, 0x01, 0x00, 0x01, 0x00, 0x00, 0x00, 0x12, 0x00, 0x04, 1, 2, 3, 4]);
