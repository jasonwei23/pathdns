...
pub fn answer_ips(packet: &[u8], question_end: usize) -> SmallVec<[IpAddr; 4]> {
    ...
    for _ in 0..an {
        let Some((rr_type, _class, _ttl, rdata, rr_end)) = parse_rr(packet, pos) else {
            return ips;
        };
        match (rr_type, rr_end - rdata) {
            ...
            (28, 16) => {
                ...
                octets.copy_from_slice(&packet[rdata..rr_end]);
...
pub(super) fn parse_rr(packet: &[u8], pos: usize) -> Option<(u16, u16, u32, usize, usize)> {
...
/// Distinct RR types present in the answer section, for `response-type` rule filters.
pub fn answer_rr_types(packet: &[u8], question_end: usize) -> SmallVec<[u16; 4]> {
    ...
    let mut pos = question_end;
    for _ in 0..an {
        let Some((rr_type, _, _, _, rr_end)) = parse_rr(packet, pos) else {
            return types;
        };
...
/// QCLASS of the question section, for `response-qclass` rule filters. Reads the
/// two bytes immediately preceding the question's end (same layout as QTYPE).
pub fn question_qclass(packet: &[u8], question_end: usize) -> Option<u16> {
    if question_end < 2 || question_end > packet.len() {
        return None;
    }
    Some(u16::from_be_bytes([
        packet[question_end - 2],
