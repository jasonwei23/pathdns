...
fn read_header<R: Read>(r: &mut R) -> Result<(u8, u64)> {
    ...
    // Same cap as every other attacker/corruption-controlled length in this file
    // (see MAX_ARRAY_BYTES): `skip_exact` doesn't allocate, but without a cap a
    // crafted file could still force streaming an unbounded amount of
    // decompressed zstd output through io::sink() just to discard it — a cheap
