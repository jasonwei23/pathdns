...
fn apply_domain_pattern(matchers: &mut TagMatchers, pattern: &str) -> bool {
    if let Some(rest) = pattern.strip_prefix("+.") {
        ...
        if let Some(n) = crate::domain::normalize_domain(rest) {
            matchers.insert_suffix(n, ());
            return true;
        }
    } else if let Some(rest) = pattern.strip_prefix('.') {
        // Subdomains only, no apex — approximated as a suffix match (see module docs).
        if let Some(n) = crate::domain::normalize_domain(rest) {
            matchers.insert_suffix(n, ());
            return true;
        }
        ...
        return true;
    } else if let Some(n) = crate::domain::normalize_domain(pattern) {
