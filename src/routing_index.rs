...
pub struct RouteIndex {
    ...
    /// Uses the qname string as key to eliminate hash-collision false hits.
...
impl RouteIndex {
    ...
    pub fn build(groups: &[CustomGroup]) -> Self {
        ...
        // 32 k entries covers the long tail of distinct queried domains without
        // significant memory cost (~a few MB at typical qname lengths).
    ...
    pub fn route<'a>(
        ...
        if use_cache {
            // Moka supports Borrow-based lookup, so &str works without allocating.
            ...
            let idx = result
                ...
                .unwrap_or(usize::MAX);
            // Allocate the key only on cache miss (once per unique qname).
