...
impl UpstreamPool {
        ...
        for (idx, endpoint) in addrs.iter().cloned().enumerate() {
            ...
            let transport = match endpoint.proto {
                ...
                UpstreamProto::Https => {
                    ...
                    UpstreamTransport::Doh(Arc::new(DoHUpstream::new(common, server_name, path)?))
                }
                UpstreamProto::Quic => {
                    let server_name = endpoint
                        .server_name
    ...
    /// Tear down every UDP node's background recv/send tasks and sockets.
    ///
    /// Only UDP needs this: its transport eagerly spawns a recv-supervisor task
    /// per pooled socket at construction time, and each task holds a strong
    /// `Arc<UdpUpstream>` that keeps it (and its open socket) alive forever with
    /// no self-terminating signal once the pool is orphaned by a config reload.
    /// TCP/TLS/DoQ/H3/DoH connect lazily on first use and their reader/writer
    /// tasks hold only per-connection state (not a strong ref to the pool
    /// itself), so an orphaned pool for those transports has no live task to
    /// leak and needs no explicit teardown here.
    pub(crate) fn shutdown(&self) {
        for node in &self.nodes {
            if let UpstreamTransport::Udp(u) = &node.transport {
                u.shutdown();
            }
        }
    }
