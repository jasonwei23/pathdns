...
pub(super) struct TcpMux {
    ...
    /// Epoch-ms before which another `force_reconnect` attempt is rejected.
    refresh_not_before_ms: AtomicU64,
    /// Guards against triggering more than one force-reconnect concurrently.
...
impl TcpMux {
    ...
    pub(super) fn force_reconnect(self: &Arc<Self>) {
        ...
        if self
            ...
            .is_err()
        {
            return; // a force-reconnect is already in flight
        }
