...
/// Shared `ConnSlot` health check for both DoQ and H3 (via its `.quic` field):
/// alive unless the QUIC connection has closed. `close_reason()` is a
/// synchronous, non-blocking check, so this wraps it in an already-ready future
/// to fit `ConnSlot`'s async `is_healthy` shape.
#[cfg(feature = "doq")]
fn is_quic_conn_healthy(c: &quinn::Connection) -> impl std::future::Future<Output = bool> {
    let alive = c.close_reason().is_none();
    async move { alive }
}
...
pub(super) struct DoQUpstream {
    ...
    /// Active QUIC connection; empty when not connected or after error.
    connection: super::ConnSlot<quinn::Connection>,
}
...
impl DoQUpstream {
    pub(super) fn new(common: super::UpstreamCommonConfig, server_name: String) -> Result<Self> {
        ...
        Ok(Self {
            ...
            connection: super::ConnSlot::new(),
        })
    }

    async fn get_or_connect(&self) -> Result<quinn::Connection> {
        self.connection
            .get_or_connect(is_quic_conn_healthy, async {
                let connecting = self
                    .endpoint
...
pub(super) struct H3Upstream {
    ...
    next_id: AtomicU32,
    inflight: Arc<tokio::sync::Semaphore>,
    connection: super::ConnSlot<H3Conn>,
}
...
impl H3Upstream {
    ...
    async fn do_exchange(&self, pkt: Bytes) -> Result<Vec<u8>> {
        ...
        if result.is_err() {
            self.connection
                .evict_if_unhealthy(|c: &H3Conn| is_quic_conn_healthy(&c.quic))
                .await;
        }
