use super::{mix16, random_id_seed, UpstreamRequest};
...
impl DoHUpstream {
    ...
    pub(super) async fn exchange(&self, req: UpstreamRequest) -> Result<Bytes> {
        ...
        // DoH runs over TLS (authenticated, off-path-spoof-proof), so no 0x20 mixing.
        let body = Bytes::from(super::prepare_query(&req.packet, &self.ecs_mode, upstream_id)?);
