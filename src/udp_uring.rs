...
// (default 256). Each holds one query plus its recvmsg header, address and
// control area, so memory per shard ≈ bufs * (MAX_PKT + ~272) ≈ 0.57 MiB at the
// default — this scales with bind addresses × interfaces × worker-threads, since
// each gets its own shard and ring, so it adds up fast on multi-homed setups.
// A too-small ring lets the multishot recvmsg stream exhaust its buffers and
// end under sustained high-outstanding-query load, forcing a re-arm (an extra
// io_uring_enter syscall) more often than necessary; raise this further for
// very high sustained QPS if that matters more to you than the extra memory.
...
impl ProvidedBufRing {
    fn new(entries: u16, buf_len: usize) -> Result<Self> {
        if !entries.is_power_of_two() {
            return Err(anyhow!(
                "io_uring buffer ring size must be a non-zero power of two"
            ));
        }
        if buf_len == 0 {
    ...
    fn buf_addr(&self, bid: u16) -> Option<u64> {
        ...
        self.bufs.get(offset).map(|byte| byte as *const u8 as u64)
    }
    ...
    fn put(&mut self, bid: u16) {
        ...
        unsafe {
            let dst = self
                .ring_mem
                .add(slot * mem::size_of::<types::BufRingEntry>());
...
struct UringRecv {
    ...
    needs_arm: bool,
    /// Which socket-level cmsgs the listen socket was configured to deliver —
    /// below `Basic` the per-packet control-area parse is skipped entirely,
    /// below `Full` the per-drain clock read / latency math is skipped.
...
impl UringRecv {
    ...
    fn drain<F: FnMut(&[u8], SocketAddr)>(&mut self, mut on_pkt: F) -> DrainStats {
        ...
        // Only needed when SO_TIMESTAMPNS cmsgs are actually being delivered.
        let now = if self.diagnostics >= UdpDiagnostics::Full {
        ...
        for cqe in &mut cq {
            ...
            if let Ok(out) = types::RecvMsgOut::parse(buf, &self.msghdr) {
                ...
                // SO_TIMESTAMPNS (kernel receive time → drain latency). With
                // diagnostics off no cmsgs were requested, so skip the walk.
                if self.diagnostics >= UdpDiagnostics::Basic {
...
fn probe() -> Option<bool> {
    ...
    let mut recv = UringRecv::new(server.as_raw_fd(), 16, 2048, UdpDiagnostics::Off).ok()?;
    recv.arm().ok()?;
