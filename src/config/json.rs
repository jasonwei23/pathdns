...
//! set → use the secondary's. Both rules are queried concurrently only to hide
//! latency — this is IP-policy routing, not a race, so `answer-ip` is required
//! in this form. `rule.filter`'s `answer-ip` match criterion (see
...
//!
//! `add-ip` still targets ipset/nftset directly (unrelated to `route.final`'s
...
//! each resolved IPv4 address, useful for route tables keyed on prefixes). The
//! v4 and v6 set names are separated by a comma in `add-ip`; use `"null"` to
//! skip one address family.
//!
//! ```json
//! {
//!   "bind": { "addr": ["0.0.0.0", "::"], "port": 53 },
...
pub(crate) struct JsonRouteSection {
    /// Rule-set data sources used by rule/answer tag matching.
    ...
    /// Domain → fixed-answer map, consulted before `rules`. Keys use the same
    /// domain patterns as `route.ruleset` (`+.`, `.`, `*.`, or bare = exact),
    /// plus `tag:`; values are a single `A://`/`AAAA://`/`CNAME://`/`RCODE://`
    /// URL or an array of them (e.g. one `A://` plus one `AAAA://`).
...
pub(crate) struct JsonRuleSetEntry {
    /// Tag name referenced by `rule.tag` / `route.answer`'s `tag:` expressions.
...
/// rules are queried concurrently (for latency), but the answer that is
...
pub(crate) struct JsonFinalSection {
    /// Rule whose answer is preferred when its IPs are in `answer-ip`.
    pub(crate) primary: Option<String>,
    /// Rule whose answer is used when the primary's IPs are NOT in `answer-ip`.
    ...
    pub(crate) answer_ip: Option<serde_json::Value>,
...
pub(crate) struct JsonRuleFilterEntry {
    ...
    pub(crate) response_type: Option<serde_json::Value>,
    /// RCODE name(s) (`"NXDOMAIN"`, …) or number(s).
    pub(crate) response_rcode: Option<serde_json::Value>,
    /// QCLASS name(s) (`"IN"`, …) or number(s).
    pub(crate) response_qclass: Option<serde_json::Value>,
    /// `"empty"` | `"drop"` | `"continue"` | `"forward"`.
    pub(crate) action: String,
    /// Target rule name; required (and only valid) when `action` is `"forward"`.
