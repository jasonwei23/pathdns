...
pub(crate) mod json;
mod upstream_url;
use self::json::{JsonBindSection, JsonConfig, JsonRuleCacheSection, JsonRuleEntry};
...
pub struct IpSetConfig {
    /// Test sets used for IP-based primary/secondary routing in `FallbackTarget::Dual`.
...
pub enum FallbackTarget {
    ...
    /// When `ipset` is `None`, both are queried in race mode (first valid wins).
    Dual {
        primary: String,
        secondary: String,
    ...
    pub(crate) fn from_json(json: JsonConfig) -> Result<Self> {
        ...
        let (fallback, verdict_cache) = if let Some(json_final) = route.route_final {
            ...
            let last = rules.last().ok_or_else(|| {
                anyhow!("route.final is required when route.rules is empty")
            })?;
            (
                FallbackConfig {
        ...
        let ipset = parse_ipset_config(&fallback, &rules)?;
            ...
            let bind: Vec<(SocketAddr, Option<String>)> = if let Some(port) = ql.port {
                ...
                match &interface {
                    InterfaceFilter::Only(ifaces) => {
                        ...
                        let unique_ips: Vec<_> = bind_addrs.iter()
                            .filter(|ep| seen.insert(ep.addr.ip()))
                            .map(|ep| ep.addr.ip())
                            .collect();
                        unique_ips.iter()
                            .flat_map(|&ip| ifaces.iter().map(move |iface| {
...
/// - `"route": {"final": {"primary":…, "secondary":…, "ipset4"/"ipset6":…}}`
///   — **ipset-test mode**: both rules are queried concurrently (for latency),
///   but the upstream is *decided by ipset membership*.  At least one of
///   `ipset4`/`ipset6` is required.
///
...
fn parse_final_config(
    ...
    let primary = jf.primary.ok_or_else(|| {
        anyhow!(
            "route.final: ipset-test mode requires \"primary\" \
             (to route to a single rule use \"route.final\": \"<rule>\")"
        )
    })?;
    let secondary = jf
        .secondary
        .ok_or_else(|| anyhow!("route.final: ipset-test mode requires \"secondary\""))?;
    if !rule_exists(&primary) {
        return Err(anyhow!("route.final.primary \"{primary}\": no such rule"));
    }
    if !rule_exists(&secondary) {
        return Err(anyhow!("route.final.secondary \"{secondary}\": no such rule"));
    }
    ...
    // Validate: mask suffix (@N for nftset, /N for ipset) belongs in add-ip, not test names.
    for (field, val) in [("ipset4", &jf.ipset4), ("ipset6", &jf.ipset6)] {
        if let Some(name) = val {
            // nftset mask: 4th '@'-segment present (family@table@set@N = 3 '@' chars)
            // ipset mask: '/' present
            let has_mask = if name.contains('@') {
            ...
            if has_mask {
                return Err(anyhow!(
                    "route.final.{field} \"{name}\": mask suffix (@N or /N) is only valid in \
                     \"add-ip\" entries. Test set names must not include a prefix mask."
                ));
            }
        }
    }
    let ipset = match (jf.ipset4, jf.ipset6) {
        (None, None) => None,
        (v4, v6) => {
            let pair = IpSetPair {
    ...
    if ipset.is_none() {
        return Err(anyhow!(
            "route.final: {{\"primary\", \"secondary\"}} requires \"ipset4\" \
             and/or \"ipset6\" — the primary's answer IPs are tested \
...
fn parse_ipset_config(
    ...
    let test = if let FallbackTarget::Dual { ipset, .. } = &fallback.target {
        ipset.clone()
    } else {
        None
    };
    ...
    if test.is_some() || !add_rules.is_empty() {
        Ok(Some(IpSetConfig { test, add_rules }))
    } else {
        Ok(None)
    }
}
...
fn parse_rule_tag_entry(value: &str) -> Result<RuleTagEntry> {
    ...
    if raw.is_empty() || is_invalid_geosite_tag(raw) {
        return Err(anyhow!(
            "rule tag: expected TAG[@ATTR] or !TAG[@ATTR], got: {value}"
        ));
    }
    // TAG@ATTR: validate both parts are non-empty.
    if let Some((base, attr)) = raw.split_once('@') {
        if base.is_empty() || attr.is_empty() {
            return Err(anyhow!(
                "rule tag: TAG@ATTR requires non-empty TAG and ATTR, got: {value}"
            ));
        }
    }
...
fn parse_rule_filter_qtype(value: Option<serde_json::Value>) -> Result<Vec<u16>> {
...
fn parse_json_rule(jg: JsonRuleEntry) -> Result<RuleSpec> {
    ...
    for tags in jg.tag.iter().flatten() {
        for entry in crate::domain::split_csv(tags) {
            match parse_rule_tag_entry(entry)? {
                RuleTagEntry::Include(t) => geosite_include.push(t),
                RuleTagEntry::Exclude(t) => geosite_exclude.push(t),
            }
        }
    }
    ...
    let filter_qtype = parse_rule_filter_qtype(jg.filter_qtype)?;
    Ok(RuleSpec {
        name: name.to_string(),
        geosite_include,
        geosite_exclude,
        ...
        cache_policy,
        filter_qtype,
        collapse: jg.collapse.unwrap_or(false),
    })
}
...
fn parse_answer_urls(
    ...
    if rcode_count == 1 {
        ...
        let (rcode_name, ttl) = split_answer_ttl(rest)
            .with_context(|| format!("route.answer entry \"{pattern}\": invalid RCODE \"{url}\""))?;
    ...
    let a_count = urls.iter().filter(|u| u.to_ascii_uppercase().starts_with("A://")).count();
    let aaaa_count = urls.iter().filter(|u| u.to_ascii_uppercase().starts_with("AAAA://")).count();
    let cname_count = urls.iter().filter(|u| u.to_ascii_uppercase().starts_with("CNAME://")).count();
    if a_count > 1 {
        return Err(anyhow!("route.answer entry \"{pattern}\": only one A:// is allowed"));
    }
    if aaaa_count > 1 {
        return Err(anyhow!("route.answer entry \"{pattern}\": only one AAAA:// is allowed"));
    }
    if cname_count > 1 {
        return Err(anyhow!("route.answer entry \"{pattern}\": only one CNAME:// is allowed"));
    }
...
fn parse_bind_config(bind: Option<JsonBindSection>) -> Result<(Vec<BindEndpoint>, InterfaceFilter)> {
...
/// Covers: rules (name, tags, filter_qtype, cache policy), fallback routing, global
...
fn split_answer_ttl(rest: &str) -> Result<(&str, u32)> {
    ...
    for param in query.split('&').filter(|p| !p.is_empty()) {
        ...
        if ttl > 2_147_483_647 {
            return Err(anyhow!("?ttl={ttl} exceeds the maximum DNS TTL (2147483647)"));
        }
    }
    Ok((value, ttl))
}
...
fn parse_fixed_answer(url: &str) -> Result<FixedAnswer> {
    ...
    match scheme.to_ascii_uppercase().as_str() {
        "A" => {
            let addr: Ipv4Addr = value.trim().parse().with_context(|| {
                format!("A://: expected an IPv4 address, got '{value}'")
            })?;
            Ok(FixedAnswer::A(addr, ttl))
        }
        "AAAA" => {
            let addr: Ipv6Addr = value.trim().parse().with_context(|| {
                format!("AAAA://: expected an IPv6 address, got '{value}'")
            })?;
...
mod querylog_tests {
    ...
    fn present_dashboard_uses_safe_defaults() {
        let cfg = parse(r#"{"route":{"rules":[{"name":"r","upstream":["1.1.1.1"]}]},"dashboard":{}}"#).unwrap();
        assert!(cfg.dashboard.enabled);
    ...
    fn invalid_dashboard_limits_are_rejected() {
        assert!(parse(r#"{"route":{"rules":[{"name":"r","upstream":["1.1.1.1"]}]},"dashboard":{"channel":0}}"#).is_err());
    ...
    fn bind_object_rejects_invalid_proto() {
        assert!(parse(r#"{"route":{"rules":[{"name":"r","upstream":["1.1.1.1"]}]},"bind":{"proto":"bogus"}}"#).is_err());
    }
    ...
    fn route_final_null_is_just_a_missing_rule() {
        ...
        assert!(parse(r#"{"route":{"rules":[{"name":"a","upstream":["1.1.1.1"]}],"final":"null"}}"#).is_err());
    }

    #[test]
    fn route_final_string_rule() {
        let cfg = parse(
            r#"{"route":{"rules":[{"name":"a","upstream":["1.1.1.1"]}],"final":"a"}}"#,
        )
        .unwrap();
    ...
    fn dashboard_port_zero_is_rejected() {
        assert!(parse(r#"{"route":{"rules":[{"name":"r","upstream":["1.1.1.1"]}]},"dashboard":{"port":0}}"#).is_err());
    }
    ...
    fn interface_empty_array_means_all() {
        let cfg = parse(r#"{"route":{"rules":[{"name":"r","upstream":["1.1.1.1"]}]},"bind":{"interface":[]}}"#).unwrap();
        assert!(matches!(cfg.interface, InterfaceFilter::All));
    }
    ...
    fn interface_bare_bang_rejected() {
        assert!(parse(r#"{"route":{"rules":[{"name":"r","upstream":["1.1.1.1"]}]},"bind":{"interface":["!"]}}"#).is_err());
    }
    ...
    fn upstream_hostname_without_bootstrap_is_error() {
        ...
        let msg = format!("{err:#}");
        assert!(msg.contains("?bootstrap="), "expected ?bootstrap= in: {msg}");
    }
    ...
    fn upstream_mark_param_parses_hex_and_decimal() {
        assert_eq!(parse_upstream("udp://1.1.1.1?mark=0x1").unwrap()[0].mark, Some(1));
        assert_eq!(
            parse_upstream("tls://8.8.8.8?mark=0xff&bootstrap=223.5.5.5").unwrap()[0].mark,
            Some(0xff)
        );
        assert_eq!(parse_upstream("udp://1.1.1.1?mark=100").unwrap()[0].mark, Some(100));
        // No ?mark= → unmarked.
        assert_eq!(parse_upstream("udp://1.1.1.1").unwrap()[0].mark, None);
        // Garbage value is rejected.
        let err = format!("{:#}", parse_upstream("udp://1.1.1.1?mark=xyz").unwrap_err());
    ...
    fn answer_ttl_param_parsed() {
        ...
        .unwrap();
        match &cfg.answer_map.lookup("a.example.com", None).unwrap().fixed_answers[0] {
            FixedAnswer::A(_, ttl) => assert_eq!(*ttl, 300),
            other => panic!("expected A, got {other:?}"),
        }
        match &cfg.answer_map.lookup("b.example.com", None).unwrap().fixed_answers[0] {
            FixedAnswer::Cname(_, ttl) => assert_eq!(*ttl, 120),
            other => panic!("expected CNAME, got {other:?}"),
        }
        assert_eq!(cfg.answer_map.lookup("c.example.com", None).unwrap().rcode_ttl, 600);
    }
