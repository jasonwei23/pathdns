...
use crate::domain::{wildcard_domain_regex, DomainMatcher};
...
impl AnswerMap {
    ...
    pub fn insert(&mut self, pattern: &str, entry: AnswerEntry) -> Result<()> {
        if let Some(rest) = pattern.strip_prefix("+.") {
            ...
            // Subdomains only, no apex — approximated as a suffix match, same
            // as route.ruleset's `.host` convention.
            self.inline.insert_suffix(normalize(rest)?, entry);
        } else if let Some(re) = wildcard_domain_regex(pattern) {
            self.inline.insert_regex(re, entry);
        } else {
            self.inline.insert_full(normalize(pattern)?, entry);
        }
        Ok(())
    }
    ...
    pub fn lookup(&self, qname: &str, ruleset: Option<&RuleSetDb>) -> Option<&AnswerEntry> {
        ...
        if let Some(rs) = ruleset {
            for (expr, e) in &self.tag {
                if expr.include.iter().any(|t| rs.matches(t, qname))
                    && !expr.exclude.iter().any(|t| rs.matches(t, qname))
                {
                    return Some(e);
                }
            }
