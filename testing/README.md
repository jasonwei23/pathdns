...
#    (run from this dir so the relative geosite path resolves)
...
Expected tail: `=== 30 passed, 0 failed ===`.

`test_match.py` reads each query's matched rule/source back from the query log
(`GET /api/querylog`), so the pathdns config must keep the dashboard enabled with
`answer-ips: true` (as `cfg_match.json` does).
