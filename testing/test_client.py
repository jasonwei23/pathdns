...
# 4. fixed-answer route.servers entry (A://)
r = q("fixed.test", 1)
check("fixed-answer A:// fixed.test -> 7.7.7.7", any(a[3]=="7.7.7.7" for a in r["answers"]),
      f"ttl={[a[2] for a in r['answers']]}")
check("fixed-answer A:// ttl=120", any(a[2]==120 for a in r["answers"]))

# 5. fixed-answer dual A+AAAA -> for A query returns A
r = q("v6.test", 1)
...
# 9. rule.filter qtype 65 (HTTPS) -> filtered (empty NOERROR, or dropped/timeout
#    depending on the matched rule's configured action)
try:
    r = q("example.com", 65, timeout=2)
    check("filter qtype 65 empty/no-answer", r["ancount"]==0, f"rcode={r['rcode']} an={r['ancount']}")
